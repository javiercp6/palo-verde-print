    ...
    >
      <div class="md:w-[40%] p-3">
        <div class="bg-base-200 rounded-lg p-3">
          <div v-if="step === 1">
            <WidgetsSelectImage />
            <LazyWidgetsSelectImageModal @pass-picture="getImageDrop2" />
...
import type { Offer, Picture } from "@/types/interface";
...
const {
  ...
  error: errorOffer,
  refresh: refreshOffer,
} = await getOfferById(route.params.id as string);
...
const getImageDrop = async (fileDrop: File) => {
  ...
  img.onload = function () {
    ...
    height.value = img.height / 150;
    width.value = img.width / 150;
...
const getImageDrop2 = async (imgUrl: string) => {
  const img = new Image();
  img.onload = function () {
    maxHeight.value = img.height / 150;
    maxWidth.value = img.width / 150;
...
watch(width, (newValue, oldName) => {
  ...
  //defaultSize;
  //const imageSize = cropp.value.getImageSize();
  cropp.value.setCoordinates({
    width: widthImg.value,
    height: heightImg.value,
    left: 0,
    top: 0,
  });
  console.log(widthImg.value, heightImg.value);
});
